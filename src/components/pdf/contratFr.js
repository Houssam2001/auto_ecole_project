...
Ce contrat est conclu entre :`, leftMargin, yPos);
  ...
  yPos += lineHeight;
  doc.text(`Numéro d'inscription au registre national des établissements d'enseignement de la conduite : ${formData.license}`, leftMargin, yPos);
  yPos += lineHeight;
  ...
  yPos += lineHeight;
  doc.text(`Téléphone :     ${formData.telephone}                                                Fax : ${formData.fax}`, leftMargin, yPos);
  yPos += lineHeight;
  doc.text(`Email:          ${formData.email}`, leftMargin, yPos);
  yPos += lineHeight;
  doc.text(`appelée "l'auto ecole" `, leftMargin+130, yPos);
  yPos += lineHeight;
  doc.text(`Nom(s) : ${data.nom} ${data.prenom}`, leftMargin, yPos);
  yPos += lineHeight;
  doc.text(`N° CIN :        ${data.CIN}                                     d'origine :   ${data.ville}                                  en date du : ${formattedDate}`, leftMargin, yPos);
  yPos += lineHeight;
  ...
  yPos += lineHeight;
  doc.text(`appelée "Condidat" `, leftMargin+130, yPos);
  yPos += lineHeight;
...
`;
  ...
  const addContentToPage = () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      yPosition += 8; // Adjust the vertical position for the next line
    }
  };
